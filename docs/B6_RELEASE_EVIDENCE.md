...
After a controlled synthetic OpenRouter attempt reached the local whole-document gate, the previous single `validation-failed` state was split into bounded fail-closed categories: missing output, truncated output, JSON structure, source alias, content policy, and Chinese-character count. These categories contain no response text, request text, aliases, counts, HTTP details, or Provider payload. They are diagnostic evidence only; a real successful completion is still required before Provider end-to-end acceptance.
