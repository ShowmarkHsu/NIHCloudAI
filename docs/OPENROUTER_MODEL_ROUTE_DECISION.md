...
documentation. The public Models API also confirms this model supports the
fixed `seed`, `temperature`, and `top_p` parameters; the prior candidate did
not, so it was immediately replaced before controlled validation. The decision
does not constitute clinical acceptance, real-data validation, artifact
