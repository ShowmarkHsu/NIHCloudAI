...
export const FIXED_FIVE_SECTION_ALLERGY_EVIDENCE_POLICY =
  'allergy-negative-findings-require-explicit-sealed-no-known-allergy-fact-v2;noneCharacterLimit=0;unsupportedAllergyWording=forbidden' as const;
...
export const FIXED_FIVE_SECTION_SYSTEM_PROMPT = [
  ...
  `過敏專則（${FIXED_FIVE_SECTION_ALLERGY_EVIDENCE_POLICY}）：只有 factTables 中 sourceFamily=allergy 且明示 no-known-allergy 的 fact，才可輸出受控的無已知過敏措辭；沒有該 fact 時，content 中「無」字元數必須為 0，不得輸出無過敏、無已知過敏或任何過敏陰性判定，必須使用 local-rendered 占位。`,
