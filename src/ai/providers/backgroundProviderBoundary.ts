  ...
  return Object.freeze({
    ...
    async generate(scope: RevisionScope, provider: SummaryProvider, request: SealedSummaryRequest): Promise<ProviderGenerationResult> {
      ...
      try {
        ...
        if (!response.ok) {
          if (provider === 'ollama' && (response.status === 400 || response.status === 404 || response.status === 413 || response.status === 422)) {
            return {status: `provider-http-${response.status}-failed` as 'provider-http-400-failed' | 'provider-http-404-failed' | 'provider-http-413-failed' | 'provider-http-422-failed'};
          }
          return {
            status: response.status >= 400 && response.status < 500
              ? `provider-http-${response.status}-failed`
              : 'provider-http-5xx-failed',
          };
        }
        let payload: unknown;
        try {
